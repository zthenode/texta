...
public:
    ...
    virtual EvError Initialize(EvServletServer& server) 
    {
        ...
        CEvHttpServletAllocatorList& actionServletList
        = CEvHttpServletAllocatorList::GetInstance();
        CEvHttpPathServletAllocatorList& pathServletList
        = CEvHttpPathServletAllocatorList::GetInstance();

        DBF("() in...\n");
        if (!(error = InitializeServlets
            (m_actionServlets, actionServletList, server)))
            error = InitializeServlets
            (m_pathServlets, pathServletList, server);
        DBF("() ...out\n");
        return error;
    }
    ///////////////////////////////////////////////////////////////////////
     ...
     EvServletServer& server) 
    {
        ...
        for (allocator = allocators.GetFirstAllocator(allocatorItem); 
             allocator; allocator = allocators.GetNextAllocator(allocatorItem))
        {
            EvcpServlet* servlet;
