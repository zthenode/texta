...
public:
    ...
    typedef EvError (CDerives::*MOnDo)
    (const char* doArgument,
...
#if defined(WIN32) 
// Windows
        if (&m_stdOut == m_writer)
        if ((error2 = m_stdOut.SetBinaryMode()))
        {
            DBE("() failed to set stdout to binary mode\n");
            return 1;
        }
#endif // defined(WIN32)
        ...
        case EvFileSystemEntry::EV_TYPE_DIRECTORY:
            ...
            if (0 < (type = m_fileEntryReader.Read
                (chars, chars2, m_isRecursive, m_isReflective)))
                error2 = OnDoEnd
                (EVCP_HEADER_FIELD_VALUE_CONNECTION_CLOSE);
            break;
     ...
     bool isBinary) 
    {
             ...
             :EVCP_FILE_MODE_READ_TEXT)))
        {
                   ...
                   (buffer, m_blockSize)))
            {
                ...
                if (blockLength >= m_blockSize)
                    continue;
                else m_putRequest.RemoveFields();
                break;
            }
            else m_putRequest.RemoveFields();
            m_file.Close();
     ...
     bool isBinary) 
    {
             ...
             :EVCP_FILE_MODE_READ_TEXT)))
        {
                   ...
                   (buffer, m_blockSize)))
            {
                ...
                if (blockLength >= m_blockSize)
                    continue;
                else m_getResponse.RemoveFields();
                break;
            }
            else m_getResponse.RemoveFields();
            m_file.Close();
    ...
    //     Date: 9/13/2007
    ///////////////////////////////////////////////////////////////////////
    virtual EvError OnDoBegin
    (const char* connection) 
    {
        EvError error = OnDo
        (EVCP_REQUEST_ACTION_ARGUMENT_NAME_BEGIN, connection);
        return error;
    }
    ///////////////////////////////////////////////////////////////////////
    // Function: OnDoCancel
    ...
    //     Date: 9/13/2007
    ///////////////////////////////////////////////////////////////////////
    virtual EvError OnDoCancel
    (const char* connection) 
    {
        EvError error = OnDo
        (EVCP_REQUEST_ACTION_ARGUMENT_NAME_CANCEL, connection);
        return error;
    }
    ///////////////////////////////////////////////////////////////////////
    // Function: OnDoEnd
     ...
     bool isDoEnd=false) 
    {
        ...
        if (m_onDo)
            error = (this->*m_onDo)
            (doArgument, connection, isDoEnd);
        return error;
    }

    ///////////////////////////////////////////////////////////////////////
    // Function: OnRequestDo
    //
    //   Author: $author$
    //     Date: 9/13/2007
    ///////////////////////////////////////////////////////////////////////
     ...
     bool isDoEnd) 
    {
        ...
        if (!(error2 = m_request.SetFields
            (EVCP_REQUEST_ACTION_NAME_DO, 
             doArgument, connection)))
     ...
     bool isDoEnd) 
    {
        ...
        if (isDoEnd)
        if ((error = EV_ERROR_FAILED))
        if (!(error2 = m_response.SetFields
            (EVCP_REQUEST_ACTION_ARGUMENT_NAME_END, 
             EVCP_REQUEST_ACTION_NAME_CO, 
             EVCP_STATUS_DESC_OK, EVCP_STATUS_CODE_OK)))
        if (0 < (length = m_response.Write(*m_writer)))
        if (!(error2 = m_response.RemoveFields()))
            error = EV_ERROR_NONE;
